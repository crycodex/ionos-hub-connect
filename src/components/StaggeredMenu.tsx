...
export const StaggeredMenu: React.FC<StaggeredMenuProps> = ({
  ...
  const animateColor = useCallback(
    (opening: boolean) => {
      // Color será manejado por CSS ahora
      return;
    },
    []
  );
        ...
        >
          <div className="sm-panel-inner flex-1 flex flex-col gap-8">
            ...
            >
              {items && items.length ? (
                  ...
                  <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={it.label + idx}>
                    {it.onClick ? (
                      <button
                        ...
                        className="sm-panel-item relative text-foreground font-medium text-[2rem] md:text-[2.5rem] cursor-pointer leading-tight tracking-normal capitalize transition-[color] duration-150 ease-linear inline-block no-underline pr-[2em] bg-transparent border-none text-left w-full"
                      ...
                      <a
                        className="sm-panel-item relative text-foreground font-medium text-[2rem] md:text-[2.5rem] cursor-pointer leading-tight tracking-normal capitalize transition-[color] duration-150 ease-linear inline-block no-underline pr-[2em]"
                        href={it.link}
                        aria-label={it.ariaLabel}
                        data-index={idx + 1}
                ...
                <li className="sm-panel-itemWrap relative overflow-hidden leading-none" aria-hidden="true">
                  <span className="sm-panel-item relative text-foreground font-medium text-[2rem] md:text-[2.5rem] cursor-pointer leading-tight tracking-normal capitalize transition-[color] duration-150 ease-linear inline-block no-underline pr-[2em]">
                    <span className="sm-panel-itemLabel inline-block [transform-origin:50%_100%] will-change-transform">
                      No items
                    </span>
                  </span>
            ...
            {displaySocials && socialItems && socialItems.length > 0 && (
              <div className="sm-socials mt-auto pt-8 border-t border-border/30 flex flex-col gap-4" aria-label="Social links">
                <h3 className="sm-socials-title m-0 text-sm font-medium text-muted-foreground uppercase tracking-wide">Todos los derechos reservados</h3>
              </div>
            )}
          </div>
        </aside>
...
.sm-scope .staggered-menu-panel { position: absolute; top: 0; right: 0; width: clamp(320px, 45vw, 500px); height: 100%; display: flex; flex-direction: column; padding: 6em 3em 3em 3em; overflow-y: auto; z-index: 10; }
.sm-scope [data-position='left'] .staggered-menu-panel { right: auto; left: 0; }
.sm-scope .sm-prelayers { position: absolute; top: 0; right: 0; bottom: 0; width: clamp(320px, 45vw, 500px); pointer-events: none; z-index: 5; }
.sm-scope [data-position='left'] .sm-prelayers { right: auto; left: 0; }
.sm-scope .sm-prelayer { position: absolute; top: 0; right: 0; height: 100%; width: 100%; transform: translateX(0); }
.sm-scope .sm-panel-inner { flex: 1; display: flex; flex-direction: column; gap: 1.25rem; }
.sm-scope .sm-socials { margin-top: auto; padding-top: 2rem; display: flex; flex-direction: column; gap: 0.75rem; }
.sm-scope .sm-socials-title { margin: 0; font-size: 1rem; font-weight: 500; color: var(--sm-accent, #00A8E8); }
...
.sm-scope .sm-socials-link:focus-visible { outline: 2px solid var(--sm-accent, #00A8E8); outline-offset: 3px; }
...
.sm-scope .sm-socials-link:hover { color: var(--sm-accent, #00A8E8); }
.sm-scope .sm-panel-list { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 0.25rem; }
.sm-scope .sm-panel-item { position: relative; font-weight: 500; cursor: pointer; line-height: 1.2; letter-spacing: 0; text-transform: capitalize; transition: color 0.25s ease; display: inline-block; text-decoration: none; padding: 0.5rem 0; padding-right: 2em; text-align: left; }
.sm-scope .sm-panel-itemLabel { display: inline-block; will-change: transform; transform-origin: 50% 100%; }
.sm-scope .sm-panel-item:hover { color: var(--sm-accent, #00A8E8); }
.sm-scope .sm-panel-item:focus-visible { outline: 2px solid var(--sm-accent, #00A8E8); outline-offset: 4px; border-radius: 4px; }
.sm-scope .sm-panel-list[data-numbering] { counter-reset: smItem; }
.sm-scope .sm-panel-list[data-numbering] .sm-panel-item::after { counter-increment: smItem; content: counter(smItem, decimal-leading-zero); position: absolute; top: 50%; transform: translateY(-50%); right: 0.5em; font-size: 0.875rem; font-weight: 400; color: var(--sm-accent, #00A8E8); letter-spacing: 0; pointer-events: none; user-select: none; opacity: var(--sm-num-opacity, 0); }
